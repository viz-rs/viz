...
fn horizontalRoundedBoxShadow(x: f32, y: f32, blur_radius: f32, corner: f32, half_size: vec2<f32>) -> f32 {
...
fn roundedBoxShadow(
    ...
    let start = clamp(-3. * blur_radius, low, high);
    let end = clamp(3. * blur_radius, low, high);
    let step = (end - start) / f32(SAMPLES);
    var y = start + step * 0.5;
    var value: f32 = 0.0;
    for (var i = 0; i < SAMPLES; i++) {
        let corner = selectCorner(p, corners);
        value += horizontalRoundedBoxShadow(p.x, p.y - y, blur_radius, corner, half_size) * gaussian(y, blur_radius) * step;
        y += step;
    }
    return value;
}
...
@vertex
fn vertex(vertex: Vertex) -> BoxShadowVertexOutput {
    var out: BoxShadowVertexOutput;
    out.position = view.clip_from_world * vec4(vertex.position, 1.0);
    out.point = (vertex.uv.xy - 0.5) * vertex.bounds;
    out.color = vertex.color;
    out.size = vertex.size;
    out.radius = vertex.radius;
    out.blur_radius = vertex.blur_radius;
    return out;
}
...
fn fragment(
    ...
    let g = in.color.a * roundedBoxShadow(-0.5 * in.size, 0.5 * in.size, in.point, max(in.blur_radius, 0.01), in.radius);
