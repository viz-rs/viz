...
use bevy_ecs::{
    change_detection::{DetectChanges, Ref, Res},
    component::Component,
    prelude::ReflectComponent,
    query::{With, Without},
    system::Query,
};
use bevy_image::{TRANSPARENT_IMAGE_HANDLE, prelude::*};
...
use crate::{
    Node,
    measure::{ContentSize, Measure, MeasureArgs},
};
...
impl ImageNode {
    ...
    pub fn is_empty(&self) -> bool {
        return self.color.is_fully_transparent()
            || self.image.id() == TRANSPARENT_IMAGE_HANDLE.id()
            || self.image_mode.uses_slices();
    }
}
...
impl Measure for ImageMeasure {
    fn measure(&mut self, args: MeasureArgs, style: &taffy::Style) -> Vec2 {
...
type UpdateImageFilter = (With<Node>, Without<crate::elements::text::Text>);
